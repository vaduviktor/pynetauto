...
def get_vnfs(bpo):
    ...
    """
    # get dnfvi domain and dnfvi devices
    dnfvi_domain = bpo.get_domains(q=f'domainType:urn:ciena:bp:domain:dnfvi')["Result"]
    domain_id = dnfvi_domain["items"][0]["id"]
    dnfvi_dev_prod = bpo.get_products_in_domain(domain_id, q='title:dnfvi Device')["Result"]
    print("Getting devices ...")
    dnfvi_devices = bpo.get_resources(productId=dnfvi_dev_prod["items"][0]["id"])["Result"]
    print("Done")

    # get VNFs
    dnfvi_vnf_prod = bpo.get_products_in_domain(domain_id, q='title:dnfvi VNF')["Result"]
    dnfvi_vnf_prod_id = dnfvi_vnf_prod["items"][0]["id"]

    dev_vnf_list = []

    # go through reachable devices and get vnfs
    print("Getting VNFs per device")
    for item in dnfvi_devices["items"]:
        ...
        dev_vnfs = bpo.get_resource_dependents(item["id"], productId=dnfvi_vnf_prod_id)["Result"]
...
def cross_compare(bpo, dev_vnf_list):
    ...
    ra_domain = bpo.get_domains(q=f'domainType:WorkflowManager')["Result"]
    ns_info_prod = bpo.get_products_in_domain(ra_domain["items"][0]["id"], q='title:Ns Info')["Result"]
    print("Getting instantiations")
    instantiations = bpo.get_resources(productId=ns_info_prod["items"][0]["id"])["Result"]
    print("Done")

    # go through instantiations
    print("Getting instantiation operation and performing cross compare with dnfvi device")
    for instance in instantiations["items"]:
        ...
        time.sleep(3)  # slow down request rate
        operations = bpo.get_resource_operation(instance["id"], q='interface:instantiateNs')["Result"]
...
def excel_out(dev_vnf_list):
    ...
    print(f'Done ... report : REPORT_{bpo_details["bpo_tenant"]}.xlsx : created')
